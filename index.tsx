...
// Import React and ReactDOM to avoid UMD global errors
...
const ARTICLES = [
  ...
  },
  {
    ...
    content: "‘断舍离’不仅仅是一种家务技巧，更是一种心智的洗礼。当我们丢弃那些无用、不爱、不再适合的物品时，本质上是在整理我们的内心边界。清理空间的过程，其实是一个不断问询自我、确认需求的过程。\n\n尝试从一个小抽屉开始。清空它，擦拭它，只放回那些真正令你心动的物品。",
    category: "心智调护",
    readTime: "6",
      ...
      <nav className="flex space-x-8">
        {[
          ...
          <button 
            ...
            className={`text-[11px] font-bold uppercase tracking-widest transition-all ${activeTab === item.id ? 'text-stone-900 border-b-2 border-stone-900 pb-1' : 'text-stone-400 hover:text-stone-600'}`}
          >
            {item.label}
          </button>
        ))}
...
const AIAdvisor = () => {
  const [msgs, setMsgs] = React.useState([{ role: 'model', content: '您好，我是您的健康顾问。有什么我可以帮您的？' }]);
  ...
  const handleSend = async () => {
    ...
    try {
      // Use named parameter for GoogleGenAI initialization and process.env.API_KEY directly
      ...
      setMsgs([...newMsgs, { role: 'model', content: response.text }]);
    } catch (e) {
      setMsgs([...newMsgs, { role: 'model', content: '（演示模式）感谢您的咨询。由于未检测到 API Key，我暂时无法进行深度对话，请在本地环境配置。' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto h-[550px] flex flex-col bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
      <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
        ...
        {loading && <div className="text-stone-300 text-[10px] italic">思考中...</div>}
      </div>
      <div className="p-6 border-t border-stone-50 bg-stone-50/20 flex gap-3">
        <input 
          ...
          placeholder="问问节气养生..."
          className="flex-1 bg-white border border-stone-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-stone-400"
        />
        <button onClick={handleSend} className="bg-stone-800 text-white px-5 rounded-xl text-[10px] font-bold uppercase">发送</button>
      </div>
    </div>
  );
};
    ...
    <div className="min-h-screen flex flex-col">
        ...
        {activeTab === 'home' && (
          <div className="space-y-20 animate-in fade-in duration-700">
            <section className="text-center py-12">
              <h1 className="serif text-5xl md:text-7xl text-stone-800 tracking-tight">呼吸间的智慧</h1>
              <p className="text-stone-400 italic mt-4 text-sm">“平素之问，探讨生命的本质”</p>
            </section>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              {ARTICLES.map((art, i) => (
                <div key={i} className="group cursor-pointer space-y-6" onClick={() => setSelectedArt(art)}>
                  <div className="aspect-[4/5] overflow-hidden rounded-2xl shadow-sm transition-all duration-700 group-hover:shadow-xl">
                    <img src={art.imageUrl} className="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 transition-transform duration-1000 group-hover:scale-105" alt={art.title} />
                  </div>
        ...
        {activeTab === 'about' && (
          <div className="max-w-2xl mx-auto text-center space-y-12 py-10 animate-in fade-in duration-700">
            ...
            <div className="space-y-6">
              <h2 className="serif text-4xl text-stone-800">关于素问</h2>
              <p className="text-stone-500 font-light leading-loose">
                《素问》源于中医经典，意为“平素之问”。我们相信，健康源于对生命节律的深刻理解。在这个快节奏的时代，我们提倡“减法养生”，回归呼吸，回归自然。
              </p>
            </div>
          </div>
        )}
      ...
      {selectedArt && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-stone-900/40 backdrop-blur-md" onClick={() => setSelectedArt(null)}></div>
          <div className="relative bg-[#fbfaf8] w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-3xl shadow-2xl p-8 md:p-12 custom-scrollbar animate-in zoom-in-95 duration-300">
             <button onClick={() => setSelectedArt(null)} className="absolute top-6 right-6 text-stone-400 hover:text-stone-800">
             ...
             <article className="space-y-8">
                <div className="text-center space-y-4">
                  <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">{selectedArt.category}</span>
                  <h2 className="serif text-4xl text-stone-900">{selectedArt.title}</h2>
                </div>
                <img src={selectedArt.imageUrl} className="w-full h-64 object-cover rounded-2xl shadow-inner" />
...
// --- 4. 挂载渲染 ---
// Use react-dom/client for modern React root mounting
